...
#Description: FL Adversarial Client Program for ISCX-VPN 2016 model cancelling
...
#load dataset
#x_train = np.load("x_twotrain-client 3.npy")
#y_train = np.load("y_twotrain-client 3.npy")

#x_test = np.load("x_twotest-client 3.npy")
#y_test = np.load("y_twotest-client 3.npy")
...
#x_train = np.load("x_bal-train-four-client 3.npy")
#y_train = np.load("y_bal-train-four-client 3.npy")

#x_test = np.load("x_bal-test-four-client 3.npy")
#y_test = np.load("y_bal-test-four-client 3.npy")
...
class ntcClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        '''
        #Model cancelling attack (set layers weight to zero)
        wx = np.zeros([740,6])
        bx = np.zeros(6)
        ...
        model.layers[0].set_weights([wx,bx])
        model.layers[1].set_weights([wx2,bx2])
        '''
...
fl.client.start_numpy_client(server_address="127.0.0.1:8080", client=ntcClient(client_id="client_3"))