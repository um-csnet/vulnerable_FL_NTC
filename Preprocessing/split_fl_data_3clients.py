...
while count < client_num :
    np.save('x_train-client ' + str(count + 1), x_train[count])
    np.save('y_train-client ' + str(count + 1), y_train[count])
    
    np.save('x_test-client ' + str(count + 1), x_test[count])
    np.save('y_test-client ' + str(count + 1), y_test[count])
    count = count + 1